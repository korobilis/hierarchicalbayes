function [beta_draws,sigma2_draws,gamma_draws,pi0_draws] = BayesRegr(y,X,nsave,nburn,prior,conjugate)
...
%     end
    ...
    elseif conjugate==0
    sigma2=1/gamrnd( c+0.5*n , 1./(d + 0.5*sse ) );
    end 
    
    % Sample regression coefficients beta
    beta = randn_gibbs_sep2021(y,X,Q,n,p,sigma2,conjugate);
    ...
    switch upper(prior_type)
        case 'CTS'
            [Q,kappa,xi,nu]=cts_prior(beta,1,b0,tau,kappa,xi,nu,lambda,method);
        case 'SSVS'
            [Q,gamma,tau0,tau1,pi0,kappa,xi,nu] = ssvs_prior(beta,1,tau0,tau1,pi0,kappa,xi,nu,method);
        case 'SS'
            [Q,gamma,tau,pi0,kappa,lambda,xi,nu] = sns_prior(y,X,1,beta,gamma,tau,pi0,kappa,lambda,xi,nu,method);
    end         
    end 
