...
%     s(round(T/2)+1:end,randperm(p,q)) = 1;
    ...
    theta0 = 2*(4.*rand(1,p)+1);%[1.5, 0, -1, 0, 1.2,  0, 3.2, 0];  % Initial regression coefficients
    mu = s(1,:).*theta0./2;%[1.5, 0, -1, 0, 1.2,  0, 3.2, 0];       % Long-run (unconditional) mean of regression coefficients
else
    rho = param.rho;
    s = param.s;
