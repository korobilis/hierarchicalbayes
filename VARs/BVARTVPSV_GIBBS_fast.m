...
tic;
%----------------------------------LOAD DATA----------------------------------------   
[Y,PHI,C,S] = simvardgpcon(200,10,1,2);
...
for ieq = 1:M
    Q{ieq,1}    = .01*ones(TKK+T*(ieq-1),1);
    miu{ieq,1}  = zeros(TKK+T*(ieq-1),1);  
    if prior == 1
    ...
    elseif prior == 2       
        % SSVS prior
        c0 = (0.01)^2;
        lambda{ieq,1} = 4*ones(TKK+T*(ieq-1),1);    % "local" shrinkage parameters
        pi0 = 0.25;                            % "global" shrinkage parameter
        b0 = 0.01;
    elseif prior == 3   
        % Horseshoe prior
        lambda{ieq,1} = 0.1*ones(TKK+T*(ieq-1),1);  % "local" shrinkage parameters
...
for iter = 1: (ngibbs + nburn)
    ...
    for ieq = 1:M                  
        ...
        betaD = randn_gibbs(ys,xs,miu{ieq,1},Q{ieq,1},T,TKK+T*(ieq-1),est_meth);  % sample regression coefficients
        beta = Hinv*betaD(1:TKK); beta_mat = reshape(beta,KK,T);
        BETA(:,ieq,:) = beta_mat;
        a_mat  = reshape(betaD(TKK+1:end),ieq-1,T);
        A(ieq,1:ieq-1,:) = cumsum(a_mat,2);
        ...
        elseif prior == 3
            [Q{ieq,1},~,miu{ieq,1},lambda{ieq,1},tau(ieq)] = horseshoe_prior(betaD,TKK+T*(ieq-1),lambda{ieq,1},tau(ieq));
        end
        ...
        X_new = [X_new, resid(:,ieq).*J];
    end
    
    OMEGA = zeros(M,M,T);
    for t = 1:T 
        OMEGA(:,:,t) = A(:,:,t)*diag(sigma_t(t,:))*A(:,:,t)';
    end
