function [Y_forc,BETA,SIGMA] = BVAR_GIBBS(Y,p,h,ndraws,stdata)
...
lambda = zeros(KK,M); tau = zeros(M,1);
...
%     end
    ...
    BETA = zeros(KK,M);
    parfor ieq = 1:M
        [BETA(:,ieq),lambda(:,ieq),tau(ieq),sigma_sq(ieq)] = horseshoe(y_til(:,ieq),X,XX,lambda(:,ieq),tau(ieq),sigma_sq(ieq),ieq,est_meth);
...
SIGMA = squeeze(median(OMEGA_save));
