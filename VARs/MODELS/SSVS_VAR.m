function [Y_forc,alpha_mat,psi_mat,sigma_mat,gamma_mat,omega_mat]  = SSVS_VAR(y,stdata,p,delta,V_prior,ndraws,forc_h)
...
tau_0 = 1/sqrt(V_prior);   %*ones(m,1);  % Set tau_[0i], tau_[1i]
tau_1 = sqrt(V_prior);     %*ones(m,1);

% Variances for the "Eta mixture", see eq.(16)  
kappa_0 = 1/sqrt(V_prior);      %*ones(m,M-1); % Set kappa_[0ij], kappa_[1ij]
kappa_1 = sqrt(V_prior);       %*ones(m,M-1);
