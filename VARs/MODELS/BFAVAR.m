...
BIC4lags  = NaN(p,1);
Y_forc    = zeros(ndraws,numel(series_to_eval)+N,m_max-m_min+1,h);
...
%     end   
    ...
    % -------| STEP 3: Prediction
    Y_pred = zeros(ndraws,N + numel(series_to_eval),h); % Matrix to save prediction draws
    ...
    for ii = 1:h % not very efficient, By^(ii-1) can be defined once
        ...
        if ndraws > 1
            ...
            Y_pred(:,:,ii) = (L*(repmat(FORECASTS(1:M),1,ndraws) +  chol(VAR_VAR(1:M,1:M))'*randn(M,ndraws)))';
        else
            Y_pred(:,:,ii) = (L*repmat(FORECASTS(1:M),1,ndraws))';
        end
        BB = BB*By;
    end
